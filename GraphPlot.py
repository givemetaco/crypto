        ...
        if self.current_type == 'line':
            ...
            if self.current_period == 'd':
                ...
                ohlc = self.coin.get_day_data()[[
                    'Open', 'High', 'Low', 'Close']].astype(float).values
                dohlc = np.hstack((np.reshape(x, (-1, 1)), ohlc))
                self.c_axes[1].bar(x, self.coin.get_day_data().Volume, color='k',
                ...
                ohlc = self.coin.get_weekly_data()[[
                    'Open', 'High', 'Low', 'Close']].astype(float).values
                dohlc = np.hstack((np.reshape(x, (-1, 1)), ohlc))
                self.c_axes[1].bar(x, self.coin.get_weekly_data().Volume, color='k',
                                   width=0.6, align='center')
            elif self.current_period == 'y':
                ...
                ohlc = self.coin.get_monthly_data()[[
                    'Open', 'High', 'Low', 'Close']].astype(float).values
                dohlc = np.hstack((np.reshape(x, (-1, 1)), ohlc))
                self.c_axes[1].bar(x, self.coin.get_monthly_data().Volume, color='k',
                                   width=0.6, align='center')

            candlestick_ohlc(self.c_axes[0], dohlc,
